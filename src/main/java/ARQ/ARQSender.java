...
public class ARQSender {
    ...
    private HashMap<Long, Timer> packetTimers = new HashMap<>();
    ...
    class ResendTask extends TimerTask
    {
        ...
        public void run()
        {
            try {
                ...
                packetTimers.put(packet.getSequenceNumber(), timer);
    ...
    private void setPacketTimer(Packet packet) {
        Timer timer = new Timer();
        ResendTask resendTask = new ResendTask(this.channel, this.routerAddress, packet);
        packetTimers.put(packet.getSequenceNumber(), timer);
        timer.schedule(resendTask, TIMER_RESEND_VALUE);
    ...
    private void sendPacket(Packet packet) throws IOException {
        ...
        this.setPacketTimer(packet);
    }

    private void cancelPacketResend(long sequenceNumber) {
        if (packetTimers.containsKey(sequenceNumber)) {
            packetTimers.get(sequenceNumber).cancel();
        }

        packetTimers.remove(sequenceNumber);
    }
    ...
    public void send(byte[] payload) throws IOException {
        ...
        while (windowIndex < sequenceNumbers.length) {
            ...
            if (acknowledgmentPacket.getSequenceNumber() == sequenceNumbers[windowIndex]) {
                ...
                this.cancelPacketResend(acknowledgmentPacket.getSequenceNumber());
                // Increment send base
                windowIndex++;
            ...
            while (receivedAcks.contains((long) sequenceNumbers[windowIndex])) {
                ...
                this.cancelPacketResend((long) sequenceNumbers[windowIndex]);
                receivedAcks.remove((long) sequenceNumbers[windowIndex++]);
        ...
        channel.receive(buf);
        buf.flip();
        packetTimers.forEach((aLong, timer) -> timer.cancel());
    }
